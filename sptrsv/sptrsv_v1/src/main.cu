...
//#include "sptrsv_syncfree_balance.h"

int main(int argc, char ** argv)
{
	if (argc < 8) {
        printf("Usage: ./test_sptrsv -n [number of GPU(s)] -rhs 1 -forward -mtx [input sparse matrix A file]\n");
		return -1;
	}
    ...
    // "Usage: ``./sptrsv -n (#gpu) -rhs 1 -forward -mtx A.mtx'' for LX=B on device 0"
    int argi = 1;
   ...
   //printf("Device [ %i ] %s @ %4.2f MHz\n", device_id, deviceProp.name, deviceProp.clockRate * 1e-3f);
    ...
    //                   substitution, rhs, OPT_WARP_AUTO, x, b, x_ref, &gflops_autotuned, ngpu);

    //printf("-----------------------------warpming up end------------------------------------------------------\n");
    
    //run cuda syncfree SpTRSV balance nnz

    printf("----------------------------start-----------------------------------------------------\n");
    double gflops_autotuned2 = 0;
    sptrsv_syncfree_cuda(cscColPtrTR, cscRowIdxTR, cscValTR, m, n, nnzTR,
                       substitution, rhs, OPT_WARP_AUTO, x, b, x_ref, &gflops_autotuned2, ngpu);
